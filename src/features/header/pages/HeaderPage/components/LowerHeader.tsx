...
const LowerHeader = ({
  ...
  return (
    <>
      ...
      <div
        className="
relative hidden md:flex justify-center
bg-[var(--bg-white)]
px-6 py-3
shadow-[0_8px_20px_-10px_rgba(0,0,0,0.15)]
...
${
  isOpen
    ? "text-[var(--primary)] bg-[var(--primary-light)]/20"
    : "text-[var(--text-main)] hover:text-[var(--primary)] hover:bg-[var(--primary-light)]/20"
}`}
                ...
                >
                  ...
                  <span className="relative after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 after:bg-[var(--primary)] after:transition-all after:duration-300 group-hover:after:w-full">
                    {cat.name}
                  </span>

                  {subcategories.length > 0 && (
                    <span
                      className={`transition-transform duration-300 ${isOpen ? "rotate-180 opacity-80" : "opacity-50"}`}
                    >
                      <ChevronIcon />
                    </span>
                  )}
    ...
    hidden md:block`}
                  >
                    <div
                      className="relative bg-[var(--bg-white)] rounded-2xl
border border-[var(--border-light)]
shadow-[var(--shadow-soft)] flex flex-col max-h-[420px] overflow-hidden"
                    >
                      {/* HEADER */}
                      <div className="flex items-center gap-3 px-5 pt-5 pb-3 shrink-0">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[var(--primary-light)]/20">
                          <span className="[&>svg]:!w-[18px] [&>svg]:!h-[18px] [&>svg]:!text-[var(--primary)]">
                            {iconMap[cat.name] || defaultIcon}
                          </span>
                        </div>
                        <div>
                          ...
                          {subcategories.map((sub) => (
                            <li key={sub.id} className="min-w-0">
                              <Link
                                ...
                                className="group relative block p-3 rounded-xl transition-all duration-300 hover:translate-x-1 hover:bg-[var(--primary-light)]/15"
                              >
                                <span className="absolute left-0 top-0 h-full w-[3px] bg-[var(--primary)] scale-y-0 group-hover:scale-y-100 origin-top transition-transform duration-300 rounded-l-xl"></span>

                                <div className="flex flex-col min-w-0">
                                  <span className="text-sm font-semibold text-gray-800 group-hover:text-[var(--primary)] break-words leading-snug">
                                    {sub.name}
                                  </span>
                      ...
                      <div className="px-5 pb-5 pt-3 shrink-0">
                        <Link
                          ...
                          className="!flex !items-center !justify-center !gap-2 !w-full !px-4 !py-[10px] !text-[14px] !font-semibold !leading-none !text-white !rounded-lg !bg-[var(--primary)]
hover:!bg-[var(--primary-dark)] !shadow-[var(--shadow-gold)] !shadow-md hover:!shadow-lg !transition-all"
                        >
                          View All {cat.name}
                        </Link>
                      </div>
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 z-[999] bg-[var(--bg-white)]
shadow-[var(--shadow-soft)] md:hidden"
        >
                ...
                return (
                  <div key={cat.id} className="border-b border-gray-100 pb-2">
                    <button
                      ...
                      className="w-full flex justify-between items-center py-2 font-semibold text-[var(--text-main)]"
                    >
                      <span>{cat.name}</span>
                      {subcategories.length > 0 && (
                        <span
                      ...
                      <div className="pl-3 mt-2 flex flex-col gap-2">
                        {subcategories.map((sub) => (
                          <Link
                            ...
                            to={`/category/${cat.slug}/${sub.name.toLowerCase().replace(/\s+/g, "-")}`}
                            onClick={() => setMobileMenuOpen(false)}
                            className="text-sm text-gray-600 hover:text-[var(--primary)] transition-all"
                          >
                            {sub.name}
                          </Link>
                        ))}
