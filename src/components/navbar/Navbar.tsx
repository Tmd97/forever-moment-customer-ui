...
const Navbar = () => {
  ...
  const navLinks = apiCategories.filter((cat: any) => cat.isActive).sort((a: any, b: any) => a.displayOrder - b.displayOrder).map((cat: any) => {
    const subCategories = cat.subCategories?.filter((sub: any) => sub.isActive).sort((a: any, b: any) => a.displayOrder - b.displayOrder) || [];
    ...
    return {
      ...
      categories: chunks.map((chunk, idx) => ({
        ...
        items: chunk.map((sub: any) => sub.name)
      }))
    };
  });
            ...
            <div className="relative hidden lg:block">
              ...
              {cityOpen && (
                <div className="absolute top-[calc(100%+8px)] left-0 bg-white rounded-xl shadow-[0_12px_40px_rgba(26,18,8,0.15)] border border-[#EDE0C4] z-20 overflow-hidden w-[160px]">
                  {locations.filter((loc: any) => loc.isActive).map((c: any) => (
                    <button
                      key={c.name}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-4 py-2.5 text-[0.78rem] hover:bg-[#FDFAF4] transition-colors ${c.name === selectedLocation ? "text-[#C9A84C] font-semibold" : "text-[#5C4A1E]"}`}
                    >
                      {c.name}
                    </button>
                  ))}
